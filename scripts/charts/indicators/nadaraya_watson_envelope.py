"""Nadaraya-Watson Envelope chart."""
...
OUTPUT_IMAGE = "nadaraya_watson_envelope.png"
...
def generate(output_dir: Path | None = None,
    ...
    df = pi.nadaraya_watson_envelope(
        df, source_column="Close",
        bandwidth=8.0, mult=3.0, lookback=500,
    )

    fig = overlay_figure(df)
    dt = df["Datetime"]
    upper = df["nwe_upper"].values
    lower = df["nwe_lower"].values
    middle = df["nwe_middle"].values
