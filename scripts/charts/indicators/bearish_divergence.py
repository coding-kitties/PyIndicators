...
OUTPUT_IMAGE = "bearish_divergence.png"
...
def generate(output_dir: Path | None = None,
    ...
    div_pts = df[df["bearish_divergence"] == 1]
    if not div_pts.empty:
        fig.add_trace(go.Scatter(
            x=div_pts["Datetime"], y=div_pts["High"],
            mode="markers",
    ...
    apply_layout(fig, "Bearish Divergence")
